// app/observations/index.jsx - With Proper Text Wrapping
...
export default function ObservationsScreen() {
    ...
    if (scrollViewRef.current && chipPositions.current[selectedFilter]) {
      setTimeout(() => {
        scrollViewRef.current?.scrollTo({
          x: chipPositions.current[selectedFilter] - 20,
          animated: true,
        });
      }, 100);
    }
    ...
    return (
      <View style={[styles.card, { borderLeftColor: status.color }]}>
        <View style={styles.cardHeader}>
          <View style={styles.userSection}>
            <View style={[styles.userAvatar, { backgroundColor: status.color + '20' }]}>
              <Text style={[styles.userAvatarText, { color: status.color }]}>
        ...
        <View style={styles.gorillaSection}>
          <View style={styles.gorillaIcon}>
            <Text style={styles.gorillaIconText}>🦍</Text>
          </View>
        ...
        {/* Health Status - Now with proper wrapping */}
        {item.healthStatus && item.healthStatus !== 'Not specified' && item.healthStatus !== 'N/A' && (
          <View style={styles.healthRow}>
            ...
            <Text style={styles.healthText} numberOfLines={0}>
              Health: {item.healthStatus}
            </Text>
          </View>
        )}

        {/* Notes - Now with proper wrapping */}
        {item.notes && (
  ...
  const FilterChip = ({ filter, isActive, onPress, onLayout }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      onLayout={(e) => onLayout(e, filter.id)}
    >
      {isActive ? (
        <LinearGradient
      ...
      >
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>
            {filteredObservations.length} total • {pendingCount} pending sync
          </Text>
        </View>
      </LinearGradient>

      {/* Filter Bar - Separate from gradient to prevent overlap */}
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              Use the + button on the map to add your first gorilla sighting
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  // Health Row - Fixed for long text
  healthRow: {
    flexDirection: 'row',
